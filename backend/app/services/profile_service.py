...
class ProfileService:
    ...
    def update(self, user: User, data: ProfileUpdate) -> ProfileOut:
        # Học sinh & giáo viên không được tự sửa thông tin (do trường quản lý)
        is_managed = user.role in (Role.student, Role.teacher)
        if not is_managed and data.email and data.email != user.email:
            if self.users.get_by_email(data.email):
                raise ConflictError("Email đã được sử dụng.")
            user.email = data.email
        if not is_managed and data.name is not None:
            user.full_name = data.name
        if not is_managed and data.phone is not None:
